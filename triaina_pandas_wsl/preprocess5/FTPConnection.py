...
def create_meta_tsv(taxa, p_id, p_df):
...
def download_taxa_data(p_id, row, a_id):
    output_path = row[6]
    remote_path = row[7]
    remote_size = row[9]
...
if __name__ == '__main__':
    ...
    for taxa in taxa_list:
        ...
        result_dfs = Parallel(n_jobs=6)(delayed(create_meta_tsv)(taxa, p_id, p_df) for p_id, p_df in tqdm(enumerate(dfs)))
        ...
        for a_id, df in enumerate(dfs):
            if a_id >= 294:
                ...
                while df.shape[0] != df["is_downloaded"].sum():
                    result = Parallel(n_jobs=6)(delayed(download_taxa_data)(p_id, row, a_id) for p_id, row in enumerate(tqdm(df.values)))
