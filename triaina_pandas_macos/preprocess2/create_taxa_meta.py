...
def download_summary(taxa, is_purge=False):
...
def check_is_downloaded(output_path, df, taxa, p_id=None, is_purge=False):
    output_path_final = f"{output_path}.{p_id}"
    if os.path.exists(output_path_final) and not is_purge:
        return pd.read_csv(output_path_final, sep="\t")
    df_len = len(df)
    if p_id == None:
    ...
    df = df.apply(lambda row: is_downloaded(row, ftp_con, taxa, p_id), axis=1)
    ftp_con.close()

    list = ["assembly_accession", "taxid", "species_taxid", "organism_name",
            "infraspecific_name", "ftp_path", "local_path", "is_downloaded", "remote_size"]
    view = df[list]
    df = view.copy()
    df.to_csv(output_path_final, sep="\t", index=False)
    return df
