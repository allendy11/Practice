...
/// hits are collected.
static void doPagingSearch(const SearcherPtr &searcher, const QueryPtr &query,
                           int32_t hitsPerPage, bool raw, bool interactive)
{
  ...
  while (true)
  {
    if (end > hits.size())
    {
      std::wcout << L"Only results 1 - " << hits.size() << L" of "
                 << numTotalHits
                 << L" total matching documents collected.\n";
    ...
    for (int32_t i = start; i < end; ++i)
    {
      if (raw)
      { // output raw format
        std::wcout << L"doc=" << hits[i]->doc << L" score="
                   << hits[i]->score << L"\n";
        continue;
      }
      ...
      if (!path.empty())
      {
        std::wcout << StringUtils::toString(i + 1) + L". " << path
                   << L"\n";
      ...
      {
        std::wcout
            << StringUtils::toString(i + 1) + L". No path for this document\n";
      }
    }
...
public:
  ...
  {
    std::wcout << L"doc=" << (doc + docBase) << L" score="
               << scorer->score();
  }
...
/// of their relevance.
static void doStreamingSearch(const SearcherPtr &searcher,
...
int main(int argc, char *argv[])
{
  ...
  //		std::wcout
  //				<< L"Usage: searchfiles.exe [-index dir] [-field f] [-repeat n] [-queries file] [-raw] ";
  //		std::wcout << L"[-norms field] [-paging hitsPerPage]\n\n";
  //		std::wcout
  //				<< L"Specify 'false' for hitsPerPage to use streaming instead of paging search.\n";
  //		return 1;
  //	}

  try
  {
    String index = L"/home/neuroears/test_index";
    ...
    //				repeat = StringUtils::toInt(
    //						StringUtils::toUnicode(argv[i + 1]));
    //				++i;
    ...
    while (true)
    {
      ...
      { // repeat and time as benchmark
        ...
        std::wcout << L"Time: "
                   << (MiscUtils::currentTimeMillis() - start) << L"ms\n";
      }
