...
DocumentPtr fileDocument(const String &docFile)
{
  ...
  doc->add(
      newLucene<Field>(L"path", docFile, Field::STORE_YES,
  ...
  doc->add(
      newLucene<Field>(L"modified",
                       DateTools::timeToString(FileUtils::fileModified(docFile),
                                               DateTools::RESOLUTION_MINUTE),
                       Field::STORE_YES,
                       Field::INDEX_NOT_ANALYZED));
...
void indexDocs(const IndexWriterPtr &writer, const String &sourceDir)
{
  ...
  for (HashSet<String>::iterator dirFile = dirList.begin();
       dirFile != dirList.end(); ++dirFile)
  {
    ...
    {
      std::wcout << L"Adding [" << ++docNumber << L"]: " << *dirFile
                 << L"\n";
...
int main(int argc, char *argv[])
{
  //	if (argc != 3) {
  //		std::wcout
  //				<< L"Usage: indexfiles.exe <index source dir> <lucene index dir>\n";
  //		return 1;
  //	}
  //
  //	String sourceDir(StringUtils::toUnicode(argv[1]));
  //	String indexDir(StringUtils::toUnicode(argv[2]));
  string sourceDir_str = "/home/neuroears/test";
  string indexDir_str = "/home/neuroears/test_index";
  String sourceDir = StringUtils::toUnicode(sourceDir_str);
  String indexDir = StringUtils::toUnicode(indexDir_str);
  ...
  try
  {
    ...
    std::wcout << L"Optimize time: " << optimizeDuration
               << L" milliseconds\n";

    writer->close();

    std::wcout << L"Total time: " << indexDuration + optimizeDuration
               << L" milliseconds\n";
  }
